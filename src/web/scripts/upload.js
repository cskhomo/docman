...
function page_init() {
    document.getElementById("upload_form").addEventListener("submit", check_file);
    document.getElementById("back_btn").addEventListener("click", goto_dashboard);
    check_error();
}
...
function check_file(e) {
    ...
    if (!file_input.files.length) {
        e.preventDefault();
        show_error( "Please select a PDF file.");
        return;
    }
    ...
    if (!is_pdf) {
        e.preventDefault();
        show_error("Only PDF files are allowed.");
        return;
    }
...
function check_error() {
    ...
    if (!error_param) return;

    show_error(decodeURIComponent(error_param));
